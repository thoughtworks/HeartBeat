...
//				.map(jiraColumn -> jiraColumn.getStatuses().stream()
//					.map(jiraColumnStatus -> getColumnStatusCategory(jiraColumnStatus.getSelf(), boardRequest.getToken()).getUntranslatedNames().stream()
//						.map(String::toUpperCase)).toList().stream())
//				.map()
