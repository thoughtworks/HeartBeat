...
import {IssueConnection, LinearFetch, WorkflowStateConnection} from "@linear/sdk/dist/_generated_sdk";
...
describe("verify token and return columns and users for Linear", async function () {
    before(() => {
        ...
        sinon
            ...
            .returns(Promise.resolve(linearWorkflowStates as unknown as WorkflowStateConnection));
    });
    after(() => {
        sinon.restore();
    });
