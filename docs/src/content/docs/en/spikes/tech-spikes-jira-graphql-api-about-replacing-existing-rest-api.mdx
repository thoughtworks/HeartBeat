			...
			columns{
				...
				columnStatus{
					status{
						id
						name
            category #Only key is returned, not name, but the name field is used
					}
				}
    }
  }
		...
		board{
			...
			columns{
				cards{
					...
					id
					fixVersions #Only id, but the name field is used
					issue{
						labels
						activeSprint{
							name
...
I found that Jira graphQL is being updated
