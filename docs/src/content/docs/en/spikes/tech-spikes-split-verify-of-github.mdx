...
- Api Design
```
paths: /api/v1/source-control/verify
method: post
request: {
  token: "...",
  type: "GitHub"
...
Triggered after acquiring data in the Pipeline settings module on the Metrics page

- Api Design
```
paths: /api/v1/source-control/repos/verify
method: post
request: {
...
- Exception Handler
  ...
  - 400: Unable to read target repo: Unable to read target repo, please recheck the token!

- C3
```plantuml
@startuml
skin rose
title C3 - Heartbeat - Verify token can read target repo
...
alt Verification success
    ...
    deactivate GitHubFeignClient
    activate GitHubService
    GitHubService -> RestResponseEntityExceptionHandler: throw 400 Bad Request
    deactivate GitHubService
    deactivate GitHubFeignClient
    activate RestResponseEntityExceptionHandler
    RestResponseEntityExceptionHandler -> FrontEnd: response 400 Bad Request: "Unable to read target repo"
    deactivate RestResponseEntityExceptionHandler
    ' deactivate GitHubService
    ' deactivate GitHubController
end
