...
import {
  ...
  MOCK_BOARD_URL_FOR_VERIFY_CLASSIC_JIRA,
  MOCK_BOARD_URL_FOR_VERIFY_JIRA,
  MOCK_BOARD_VERIFY_CLASSIC_JIRA_REQUEST_PARAMS,
...
const server = setupServer(
  rest.get(MOCK_BOARD_URL_FOR_VERIFY_JIRA, (req, res, ctx) => res(ctx.status(HttpStatusCode.Ok))),
  rest.get(MOCK_BOARD_URL_FOR_VERIFY_CLASSIC_JIRA, (req, res, ctx) => res(ctx.status(HttpStatusCode.Ok)))
)
...
describe('error notification', () => {
  ...
  it('should isBoardVerify is true when select classic jira and board verify response status 200', async () => {
    const result = await boardClient.getVerifyBoard(MOCK_BOARD_VERIFY_CLASSIC_JIRA_REQUEST_PARAMS)
  ...
  it('should throw error when board verify response status 400', async () => {
    server.use(rest.get(MOCK_BOARD_URL_FOR_VERIFY_JIRA, (req, res, ctx) => res(ctx.status(HttpStatusCode.BadRequest))))
  ...
  it('should throw error when board verify response status 500', async () => {
    server.use(
      rest.get(MOCK_BOARD_URL_FOR_VERIFY_JIRA, (req, res, ctx) => res(ctx.status(HttpStatusCode.InternalServerError)))
    )
